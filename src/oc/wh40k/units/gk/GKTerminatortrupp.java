...
public class GKTerminatortrupp extends Eintrag {
    ...
    public void refreshen() {
		...
		boolean thawnGlobal = ( BuildaHQ.getCountFromInformationVector("GKThawn") > 0 ? true : false );
		if(thawnGlobal && !thawnSelected) thawn.setAktiv(false);
		else thawn.setAktiv(true);

		if(thawn.isSelected()) {
			thawnSelected = true;
			BuildaHQ.addToInformationVector("GKThawn", 1);
		} else {
			if(thawnSelected) {
				thawnSelected = false;
				BuildaHQ.setInformationVectorValue("GKThawn", 0);
