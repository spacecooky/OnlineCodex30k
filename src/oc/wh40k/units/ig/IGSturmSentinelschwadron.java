...
public class IGSturmSentinelschwadron extends Eintrag {
	...
	public IGSturmSentinelschwadron() {
		name = "Sturm-Sentinelschwadron";
		grundkosten = 0;

		checkDraftOption();
		squad = new AnzahlPanel(ID, randAbstand, cnt, "Sturm-Sentinel", "Sturm-Sentinels", 1, 3, 55);
		squad.setGrundkosten(0);
		add(squad);

		add(ico = new oc.Picture("oc/wh40k/images/IGArmouredSentinelSquadron.jpg"));
	...
	public void refreshen() {
		...
		if(isDemonHunters || isWitchHunters) {
			...
			int choice = BuildaHQ.getCountFromInformationVector("IGArmouredSentinelSquadron");
			if(draftTroops < 2) {
	...
	private void checkDraftOption() {
		...
		if(isDemonHunters || isWitchHunters) {
			BuildaHQ.addToInformationVector("IGArmouredSentinelSquadron", 1);
		}
	}
