...
public class IHLegionTacticalSupportSquad extends Eintrag {
	...
	public IHLegionTacticalSupportSquad() {
		...
		BuildaHQ.addToInformationVector("IHInfantry", 1);
		...
		seperator();
		rkBoss = new RuestkammerStarter(ID, randAbstand, cnt, "IHSergeant", "Legion Sergeant");
		...
		seperator();
		rkTransportPod = new RuestkammerStarter(ID, randAbstand, cnt, "IHTransportKammerPod", "Legion Drop Pod\n");
		rkTransportPod.initKammer();
		rkTransportPod.setButtonText("Legion Drop Pod");
		add(rkTransportPod);

		seperator();
		rkTransportEagle = new RuestkammerStarter(ID, randAbstand, cnt, "IHTransportKammerEagle", "Legion Storm Eagle Assault Gunship\n");
		rkTransportEagle.initKammer();
		rkTransportEagle.setButtonText("Legion Storm Eagle Assault Gunship");
		add(rkTransportEagle);

		seperator();
		rkTransportPhobos = new RuestkammerStarter(ID, randAbstand, cnt, "IHTransportKammerPhobos", "Legion Land Raider Phobos\n");
		rkTransportPhobos.initKammer();
		rkTransportPhobos.setButtonText("Legion Land Raider Phobos");
		add(rkTransportPhobos);

		seperator();
		rkTransportProteus = new RuestkammerStarter(ID, randAbstand, cnt, "IHTransportKammerProteus", "Legion Land Raider Proteus\n");
		rkTransportProteus.initKammer();
		rkTransportProteus.setButtonText("Legion Land Raider Proteus");
		add(rkTransportProteus);

		seperator();
		rkTransportRhino = new RuestkammerStarter(ID, randAbstand, cnt, "IHTransportKammerRhino", "Legion Rhino Armoured Carrier\n");
